<link rel="stylesheet" type="text/css" href="<?php site_url();?>assets/css/entertainment.css">

<!-- Start WOWSlider.com HEAD section -->
	<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>assets/plugins/WOWSlider/engine1/style.css" />
	<script type="text/javascript" src="<?php echo base_url(); ?>assets/plugins/WOWSlider/engine1/jquery.js"></script>
	<!-- End WOWSlider.com HEAD section -->
</head>
<body style="background-color:#d7d7d7">
	<!-- Start WOWSlider.com BODY section -->
	<div id="wowslider-container1">
	<div class="ws_images"><ul>
<li><img src="<?php echo base_url(); ?>assets/plugins/WOWSlider/data1/images/headerbg.png" alt="header-bg" title="MAIN EVENT COMPFEST" id="wows1_0"/>Balairung Universitas Indonesia, 21-22 September 2013</li>
<li><img src="<?php echo base_url(); ?>assets/plugins/WOWSlider/data1/images/headerbg.png" alt="header-bg" title="MAIN EVENT COMPFEST" id="wows1_0"/></li>
<li><img src="<?php echo base_url(); ?>assets/plugins/WOWSlider/data1/images/headerbg0.gif" alt="header-bg" title="header-bg" id="wows1_1"/></li>
</ul></div>
<div class="ws_bullets"><div>
<a href="#" title="header-bg"><img src="<?php echo base_url(); ?>assets/plugins/WOWSlider/data1/tooltips/headerbg.jpg" alt="header-bg"/>1</a>
<a href="#" title="header-bg"><img src="<?php echo base_url(); ?>assets/plugins/WOWSlider/data1/tooltips/headerbg.jpg" alt="header-bg"/>1</a>
<a href="#" title="header-bg"><img src="<?php echo base_url(); ?>assets/plugins/WOWSlider/data1/tooltips/headerbg_0.jpg" alt="header-bg"/>2</a>
</div></div>

	<div class="ws_shadow"></div>
	</div>
	<script type="text/javascript" src="<?php echo base_url(); ?>assets/plugins/WOWSlider/engine1/wowslider.js"></script>
	<script type="text/javascript" src="<?php echo base_url(); ?>assets/plugins/WOWSlider/engine1/script.js"></script>
	<!-- End WOWSlider.com BODY section -->

<div id="entertain-wrapper">	
	<h2 class="title-content">Entertainment</h2>
	<div id="guest-star" class="entertain-content"> 	
		<h3 class="title-content">Guest Star</h3>
		<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/600x200.gif" />
	</div>
	<div id="band" class="entertain-content">
		<h3 class="title-content">Bands</h3>
		<table>
			<tr>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
			</tr>
			<tr>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
			</tr>
		</table>
	</div>
	<div id="perform" class="entertain-content">
		<h3 class="title-content">Performances</h3>
		<table>
			<tr>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
				<td>
					<img class="team-img" src="<?php echo base_url(); ?>assets/img/placehold/300x200.gif" />	
				</td>
			</tr>
		</table>
	</div>
</div>
